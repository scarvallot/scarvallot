## Hi there 👋

## Soy Sergio Carvallo
**Full Stack JavaScript Trainee** | **Developer** | **Backend Oriented**

Me he dedicado construir sistemas robustos, escalables y bien estructurados. Me especializo en **Backend**, el diseño de bases de datos relacionales y la arquitectura de APIs. Aunque tengo visión full stack, mi mayor fortaleza y motivación está en el lado del servidor: crear lógica de negocio sólida, modelar datos con integridad y entregar APIs limpias y mantenibles.

---

## 🛠️ **Stack Tecnológico**

- **Lenguajes y Frameworks:** JavaScript (ES6+), Node.js, Express
- **Bases de Datos:** PostgreSQL, SQL, Modelado Relacional, ORM (Sequelize / Prisma)
- **Herramientas y Arquitectura:** REST APIs, Arquitectura Modular, Git, Docker, NPM
- **Habilidades clave:** Programación asíncrona, Diseño de APIs, Lógica de Negocio, Calidad de Datos

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=for-the-badge&logo=express&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

---

## 🎯 **Focus**

Combino sólidos fundamentos técnicos con una mentalidad orientada a resultados:

- **Backend & Arquitectura:** Desarrollo de aplicaciones con Node.js + Express bajo patrones modulares y escalables.
- **Datos:** Diseño y optimización de bases de datos relacionales, asegurando integridad, rendimiento y mantenibilidad.
- **Frontend:** Interfaces claras y funcionales (HTML, CSS, JavaScript) con buen manejo de asincronía y experiencia de usuario.
- **Buenas Prácticas:** Código limpio, testing, documentación y preparación para entornos productivos.

---

## 📫 **Contacto**

- 💼 [LinkedIn](https://www.linkedin.com/in/scarvallot/)
- ✉ [s.carvallot@gmail.com](mailto:s.carvallot@gmail.com)

---

### Visitantes
![Visitor Badge](https://komarev.com/ghpvc/?username=scarvallot&color=0e75b6&style=flat&label=Visitantes)

---
